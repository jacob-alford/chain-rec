...
import { constVoid, flow, identity, tuple } from 'fp-ts/function'
...
export const fact: (fibAcc: FactAcc) => Step.Step<FactAcc, bigint> = flow(
  Step.loopIf(
    ([n]) => n > 1n,
    ([n, acc]) => tuple(n - 1n, acc * n),
  ),
  Step.mapResult(([, acc]) => acc),
  Step.mapIterant(identity),
  Step.mapBoth(identity, identity),
)

export const fact100 =
  93326215443944152681699238856266700490715968264381621468592963895217599993229915608941463976156518286253697920827223758251185210916864000000000000000000000000n

export function testFactM<M extends hkt.URIS4>(
  M: ChnRec.ChainRec4<M>,
  P: P.Pointed4<M>,
  test?: () => void,
): <S, R, E>(n: bigint) => hkt.Kind4<M, S, R, E, bigint>
export function testFactM<M extends hkt.URIS3>(
  M: ChnRec.ChainRec3<M>,
  P: P.Pointed3<M>,
  test?: () => void,
): <R, E>(n: bigint) => hkt.Kind3<M, R, E, bigint>
export function testFactM<M extends hkt.URIS3, E>(
  M: ChnRec.ChainRec3C<M, E>,
  P: P.Pointed3C<M, E>,
  test?: () => void,
): <R>(n: bigint) => hkt.Kind3<M, R, E, bigint>
export function testFactM<M extends hkt.URIS2>(
  M: ChnRec.ChainRec2<M>,
  P: P.Pointed2<M>,
  test?: () => void,
): <E>(n: bigint) => hkt.Kind2<M, E, bigint>
export function testFactM<M extends hkt.URIS2, E>(
  M: ChnRec.ChainRec2C<M, E>,
  P: P.Pointed2C<M, E>,
  test?: () => void,
): (n: bigint) => hkt.Kind2<M, E, bigint>
export function testFactM<M extends hkt.URIS>(
